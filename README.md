...
Bucket C's design has begun, starting from the project's original definition ("no single source, definitionally fuzzy") re-derived against a real example — TSMC's market-share claim — rather than built from memory of an old abstraction. The first piece, `bucket_triage.py`, is built, tested, and live-verified: a real LLM call classifies a claim as Bucket A, Bucket C, or genuinely "ambiguous" (a stable, final finding, never retried, the same character as `quote_match`'s `"ambiguous"` status) before any search or fetch happens. The design deliberately ruled out two tempting deterministic shortcuts along the way (hedge-word detection, comparative-keyword detection), each killed by a real, constructed counterexample rather than accepted on the strength of "I can't think of one." The remaining two pieces — per-source extraction (finding a claimed figure and its stated definition within one source) and reconciliation (grouping sources that share an underlying definition, which genuinely requires judgment, not string matching) — are designed in principle but not yet built; see `DESIGN_DECISIONS.md` for the full design trail.
