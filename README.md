...
Patagonia's website blocks plain HTTP fetches. A browser-mimicking fetcher would have "fixed" it — and was evaluated and rejected on ethical grounds: a function purpose-built to make automated requests indistinguishable from human browsing is structurally what bot-detection exists to catch, regardless of any single call's motivation. The system instead reports honest gaps, the evidence structure records third-party sourcing explicitly, and the limitation is documented rather than patched around. ([ADR-0012](adr/0012-nzif-live-totalenergies-patagonia.md), [KNOWN_LIMITATIONS.md](KNOWN_LIMITATIONS.md))

## Status

All four verification types are implemented, tested, and live-verified end to end — a full deterministic suite (no network, every LLM injected as a fake, CI-enforced coverage threshold) plus per-module live API tests. See [CURRENT_STATUS.md](CURRENT_STATUS.md) for the full module inventory and live-verified milestones.
