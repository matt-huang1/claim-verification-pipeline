...
Bucket C's design has begun, starting from the project's original definition ("no single source, definitionally fuzzy") re-derived against a real example — TSMC's market-share claim — rather than built from memory of an old abstraction. Two of its three pieces are now built, tested, and live-verified. `bucket_triage.py` classifies a claim as Bucket A, Bucket C, or genuinely "ambiguous" (a stable, final finding, never retried, the same character as `quote_match`'s `"ambiguous"` status) before any search or fetch happens, with two tempting deterministic shortcuts (hedge-word detection, comparative-keyword detection) deliberately killed by real, constructed counterexamples rather than accepted on the strength of "I can't think of one." `source_extraction.py` gathers multiple independent sources for a claim and, for each one, independently proposes and verifies a claimed value and its stated definition — each given its own honest "not found" option, each checked separately against the real document via `quote_match`, with a source included only if at least one of the two checks genuinely passed. Live-verified end to end on the real "TSMC has roughly 60% of the foundry market" claim, correctly returning third-party analyst sources (the expected, correct outcome for a definitionally contested claim, not a partial failure). The remaining piece, reconciliation — grouping verified sources by whether they share an underlying definition, which genuinely requires judgment rather than string matching, since "pure-play foundries" and "merchant foundry market" may describe the same real scope in different words — is designed in principle but not yet built; see `DESIGN_DECISIONS.md` for the full design trail.

Both buckets now write to one shared, structured log (`logs/evaluation_log.jsonl`) rather than Bucket A logging and Bucket B logging nothing at all — every entry, from either bucket, is tagged with `company_name` and `bucket`, so cross-bucket, cross-company history (what's worked, what's failed, for which company, at which stage) is queryable in one place without manually merging files. Making this change required adding `company_name` as a new, explicitly required parameter to `extract_claim_evidence` itself, the same "never inferred, always explicit" principle Bucket B's orchestrator already used — and, while doing so, the live test caught a real regression the deterministic suite of 141 tests had missed entirely (a missing import that only broke the real, un-mocked OpenAI call path), the same "a live run finds what a mock cannot" lesson this project keeps relearning.
